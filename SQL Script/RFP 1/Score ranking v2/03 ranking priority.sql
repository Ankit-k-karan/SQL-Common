...
update pr
set pr.category ='Ct1' from prop_mainmaster pm  inner join PROP_RANK_v2 pr on pr.prop_id = pm.id where pm.proposal_code in ('NSDC_TP_1440720180901011700'
...
update  PROP_RANK_v2 
set  allocated_target=0 where allocated_target>0
...
select pm1.applicant_pia_id, pr1.proposed_district, pr1.prop_id, pr1.rnk,pr1.proposal_priority
		, dense_rank() over(partition by  pm1.applicant_pia_id, pr1.proposed_district  ORDER BY pr1.rnk,pr1.proposal_priority,(SELECT Count(DISTINCT pt.Trade_Id)
                                                           FROM   Prop_Training_PerposedTrade pt
                                                           WHERE  pt.Is_Active = 1
                                                                  AND prop_id = pm1.id) desc,proposedtrainingcenterArea desc,pr1.prop_id) rrr
