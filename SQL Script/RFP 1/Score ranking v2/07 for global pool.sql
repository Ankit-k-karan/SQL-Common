...
Open District_1
FETCH NEXT FROM District_1 INTO @proposed_district
...
select (SELECT Count(DISTINCT pt.Trade_Id)
	 ...
	 from PROP_RANK_v2 pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id 
		 ...
		 and not exists (select 1 from PROP_RANK_v2 pr3
...
and Valid_TC = 1
and isnull(suspended_TP,0)=0 and isnull(allocated_target,0) =0
...
order by District_Category,proposed_district ,rnk,proposal_priority,(SELECT Count(DISTINCT pt.Trade_Id)
                                                           FROM   Prop_Training_PerposedTrade pt
                                                           WHERE  pt.Is_Active = 1
                                                                  AND prop_id = pm.id) desc,proposedtrainingcenterArea desc,prop_id
