--truncate table md_Sectors

select * from  md_Sectors  order by 4 
-------------------Sectors
insert into md_Sectors(Sector_Code,Sector_Name,Is_Active,Cdate,CBy)
select distinct case when s.Sector_Code is not null then s.sector_code else 
				case when sector ='Aerospace & Aviation' then 'AA'
					when sector ='Apparel Made -ups and Home Furnishing' then 'Apar'
					when sector ='Banking, Financial Services and Insurance' then 'BFSI'
					when sector ='Electronics & Hardware' then 'E&H'
					when sector ='Food Processing' then 'FP'
					when sector ='Green Jobs' then 'GJ'
					when sector ='Handicrafts & Carpets' then 'H&C'
					when sector ='Hydrocarbon' then 'Hydro'
					when sector ='Infrastructure Equipment' then 'IE'
					when sector ='Life Sciences' then 'LS'
					when sector ='Logistics' then 'LOGIS'
					when sector ='Management and Entrepreneurship & Professional' then 'MEP'
					when sector ='MESC' then 'MESC'
					when sector ='Paints and Coatings' then 'PC'
					when sector ='People with Disability*' then 'PWD'
					when sector ='Power' then 'POW'
					when sector ='Tourism and Hospitality' then 'TH' end
	end sec_code, ltrim(rtrim(sector))  sector ,1 isctive,GETDATE() dat ,1 active from (
...
union all select 'Gems & Jewellery' sector, 'Cast & Diamonds – set Jewellery–Hand sketch Designer (Basic)' QP, 'G&J/Q2301' QPREF, '3' nsqf, '190' duration ,'P' tradetype
...
)A)B left join NSDC..md_Sectors s on s.sector_name = B.sector
where not exists (select 1 from md_Sectors  s where s.Sector_Name = b.sector)
order by 2
