...
--  order by 1 
 ...
 if not exists (select * from sys.TAbles Where NAme = 'PROP_RANK_v5')
 begin
 create table PROP_RANK_v5
  ...
  truncate table PROP_RANK_v5
  insert into PROP_RANK_v5 (category,proposed_district,TotalScore,prop_id,rnk)
...
case when exists(select 1 from NSDC_Suspended_TP t where t.pia_id = p.Applicant_Pia_Id ) then 0 else TotalScore end TotalScore,case when  proposed_district =723 then 574 
 ...
 and 1 = case when (case when p.id in (31905,31815,31761,31693,31612,31042) then 'Ct1' 
  ...
  set totalscore=0
  --select *
    from prop_mainmaster p  where not exists (select 1 from PROP_RANK_v5 v where v.prop_id = p.id) and proposal_status='Submitted to NSDC' and  totalscore>0;
  --and not exists(select 1 from NSDC_Suspended_TP t where t.pia_id = p.Applicant_Pia_Id ) 
  --and trainingcentertype !='PROPOSED'

 update s
 set score=0 from scoringdetails s inner join prop_mainmaster p  on prop_id = p.id  and totalscore=0 and proposal_status='Submitted to NSDC' and score>0;
...
case when exists(select 1 from NSDC_Suspended_TP t where t.pia_id = p.Applicant_Pia_Id ) then 0 else TotalScore end TotalScore,case when  proposed_district =723 then 574 
 ...
 from prop_mainmaster p  -- 17734
 inner join md_district_category dc on dc.district_id = p.proposed_district --17734
and not exists (select 1 from PROP_RANK_v5 r where r.prop_id =p.id)
  where   proposal_status ='Submitted to NSDC') A
  order by 1,2


   select * from PROP_RANK_v5 r

  select prop_id from PROP_RANK_v5 r
  GROUP BY prop_id
  HAVING COUNT(1)>1
