...
update pr2
set pr2.global_rank=A.rnk  from PROP_RANK_v240 pr2 inner join (
select  prop_id, dense_rank() over(partition by proposed_district  ORDER BY TotalScore desc,category ) rnk 
from PROP_RANK_v240 pr2
where  new_allocated_target is  null and pr2.rnk is not null and TotalScore>0 
