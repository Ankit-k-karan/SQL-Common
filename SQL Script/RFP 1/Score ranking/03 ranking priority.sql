...
select top 10 * from PROP_RANK_v2
--update PROP_RANK_v2
...
union all 
select prop_id ,proposed_district,district_category,rnk, dense_rank() over(partition by proposed_district,rnk  ORDER BY NSDC_PARTNER desc,Community_Engagement desc) prir
  from  (
select isnull((SELECT  IS_NSDCPartner  FROM   [Pia_NSDCAssociation] PN
						      WHERE pn.Is_Active = 1 AND pm.org_id  =  pn.org_id),0) NSDC_Partner
		,isnull((select  Count(1) from  Pia_CommunityEngagementProject PN
...
--cat3
union all 
select prop_id ,proposed_district,district_category,rnk, dense_rank() over(partition by proposed_district,rnk  ORDER BY NSDC_PARTNER desc,TurnOver desc) prir
  from  (
select isnull((SELECT  IS_NSDCPartner  FROM   [Pia_NSDCAssociation] PN
						      WHERE pn.Is_Active = 1 AND pm.org_id  =  pn.org_id),0) NSDC_Partner
		,isnull((select  Count(1) from  Pia_CommunityEngagementProject PN
...
case when Nature_Of_Entity ='Non-Profit' then Community_Engagement else TurnOver end  desc) prir
  from  (
select isnull((SELECT  IS_NSDCPartner  FROM   [Pia_NSDCAssociation] PN
						      WHERE pn.Is_Active = 1 AND pm.org_id  =  pn.org_id),0) NSDC_Partner
		,isnull((select  Count(1) from  Pia_CommunityEngagementProject PN
...
where p.Proposal_Priority is null;
...
select pm1.applicant_pia_id, pr1.proposed_district, pr1.prop_id, pr1.rnk,pr1.proposal_priority
		, dense_rank() over(partition by  pm1.applicant_pia_id, pr1.proposed_district  ORDER BY pr1.rnk,pr1.proposal_priority,pr1.prop_id) rrr
 from  PROP_RANK_v2 pr1 
inner join Prop_mainmaster pm1 on pm1.id = pr1.prop_id  and pr1.Valid_TC =1
inner join (
select pm.applicant_pia_id 
		, pr.proposed_district,count(1)cnt
from PROP_RANK_v2 pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id  and pr.Valid_TC =1
