...
--select (select distinct category from ScoringDetails_v2 where prop_id = p.id)category,
...
--  order by 1 
  ...
  --update ScoringDetails_v2
  --set score ='5' where category ='Cat4b' and parameter_id ='5'

 --select distinct score from ScoringDetails_v2
 --where category ='Cat3' and parameter_id ='5'
 if not exists (select * from sys.TAbles Where NAme = 'PROP_RANK_Gen')
 begin
 create table PROP_RANK_Gen
  ...
  truncate table PROP_RANK_Gen
  insert into PROP_RANK_Gen (category,proposed_district,TotalScore,prop_id,rnk)
  select   category,proposed_district ,(select sum(score) from ScoringDetails_v2 v where v.prop_id = proposedid)TotalScore,proposedid, dense_rank() over(partition by category,proposed_district  ORDER BY (select sum(score) from ScoringDetails_v2 v where v.prop_id = proposedid) desc) rnk 
from (select case when p.id in (31905,31815,31761,31693,31612,31042) then 'Ct1' 
		else (select distinct replace(replace(category,'A',''),'B','') from ScoringDetails_v2 where prop_id = p.id)end category,
case when exists(select 1 from NSDC_Suspended_TP t where t.pia_id = p.Applicant_Pia_Id ) then 0 else TotalScore end TotalScore,case when  proposed_district =723 then 574 
 ...
 and 1 = case when (case when p.id in (31905,31815,31761,31693,31612,31042) then 'Ct1' 
		else (select distinct replace(replace(category,'A',''),'B','') from ScoringDetails_v2 where prop_id = p.id)end) = 'Ct1' then 1 
			when District_Category in (3,4,5,6,7,8) and trainingcentertype='PROPOSED' then 1 else 1 
			end
 ...
 -- select * from prop_mainmaster p  -- 17734
 --inner join md_district_category dc on dc.district_id = p.proposed_district  and p.id =43854--17734
 --and 1 = case when (case when p.id in (31905,31815,31761,31693,31612,31042) then 'Ct1' 
	--	else (select distinct replace(replace(category,'A',''),'B','') from ScoringDetails_v2 where prop_id = p.id)end) = 'Ct1' then 1 
 ...
 -- where   proposal_status ='Submitted to NSDC'
  ...
  --select * from NSDC_Suspended_TP where pia_id =14400


  ---score 0 from proposed and suspended TP proposal 
  --update p
  --set totalscore=0
  ----select *
  --  from prop_mainmaster p  where not exists (select 1 from PROP_RANK_Gen v where v.prop_id = p.id) and proposal_status='Submitted to NSDC' and  totalscore>0;
  ----and not exists(select 1 from NSDC_Suspended_TP t where t.pia_id = p.Applicant_Pia_Id ) 
  ----and trainingcentertype !='PROPOSED'

 --update s
 --set score=0 from ScoringDetails_v2 s inner join prop_mainmaster p  on prop_id = p.id  and totalscore=0 and proposal_status='Submitted to NSDC' and score>0;
...
-- from prop_mainmaster p  -- 17734
-- inner join md_district_category dc on dc.district_id = p.proposed_district --17734
--and not exists (select 1 from PROP_RANK_Gen r where r.prop_id =p.id)
--  where   proposal_status ='Submitted to NSDC') A
--  order by 1,2


  -- select * from PROP_RANK_Gen r

  --select prop_id from PROP_RANK_Gen r
  --GROUP BY prop_id
  --HAVING COUNT(1)>1
  ...
  --select * from ScoringDetails_v2 where prop_id in  (20162,20195,20218) and parameter_id ='5'