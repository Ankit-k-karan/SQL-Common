...
and isnull(allocated_target,0)=0 and pr.rnk is not null
...
select (SELECT Count(DISTINCT pt.Trade_Id)
	 ...
	 from PROP_RANK_Gen pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id and pr.rnk is not null -- and  pr.proposed_district=301
		 ...
		 and not exists (select 1 from PROP_RANK_Gen pr3
...
and Valid_TC = 1
and isnull(suspended_TP,0)=0 and  isnull(allocated_target,0)=0 --allocated_target is null
   ...
   BEGIN
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			...
			set @MaxTarget = case when @District_Category in (1,2) then 200000000  else 12000000000 end; --CHANGE 27.09.2018 --when @District_Category  =8 THEN 900 
			   set  @TargetAllocated = isnull((select sum(allocated_target) from PROP_RANK_Gen where proposed_district= @proposed_district  ),0);
			   set  @TCAllocated = isnull((select count(1) from PROP_RANK_Gen pr
