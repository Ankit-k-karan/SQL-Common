...
update v
set suspended_TP=1 from PROP_RANK_Gen v inner join prop_mainmaster p on v.prop_id = p.id
...
set p.Proposal_Priority =b.prir
 from PROP_RANK_Gen p inner join 
...
from PROP_RANK_Gen pr
		...
		update p
		set Valid_TC =0 from cte inner join PROP_RANK_Gen p on cte.prop_id = p.prop_id  where rrr>1 and Valid_TC =1