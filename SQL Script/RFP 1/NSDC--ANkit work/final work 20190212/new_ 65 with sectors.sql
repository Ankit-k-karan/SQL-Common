SELECT State_Name                                      AS [State],
       ...
       TrainingCenterTypeName                          AS [TC Type],
		...
		,ISCompleteDD as [Is in DD]
		,isnull(New_Allocated_Target,0) [New_Allocated_target]
		
			, case When isnull(New_Allocated_Target,0) >0 then isnull(New_Allocated_Target,0)
		       else   isnull(pr.allocated_target,0)	end as [Target Allocated]
		,case when ISCompleteDD = 1  then
		 case L2Status
		 when 'Submitted' then 'Submitted'
		 when 'Document Re-uploaded' then 'Information Awaited'
		 else 'Save As Draft'
		 end
		 end as [DD Status]
		 , (select STUFF((SELECT distinct ',' + sec.sector_NAME
            FROM md_Sectors Sec
						       ...
						       WHERE propTrade.Prop_Id = v.proposalid and Sec.Sector_Name IN ( 'Paints and Coatings', 'Aerospace & Aviation', 'Rubber', 'Hydrocarbon',
...
inner join PROP_RANK_Sector_240_v20190212 pr on v.proposalid = pr.prop_id
--and v.proposal_code in ('NSDC_TP_1318420180831080650','NSDC_TP_1351220180830073915','NSDC_TP_1407920180902120933','NSDC_TP_1506520180902083445')
and not exists (select 1 from  target_final vf where  pr.prop_id = vf.prop_id )
and new_allocated_target>0
...
inner join PROP_PARAM_SCORE ps on ps.prop_id = pr.prop_id 
 left JOIN Prop_InitScreening InitialScreening
                           ON InitialScreening.Proposal_Id =  v.proposalid 
