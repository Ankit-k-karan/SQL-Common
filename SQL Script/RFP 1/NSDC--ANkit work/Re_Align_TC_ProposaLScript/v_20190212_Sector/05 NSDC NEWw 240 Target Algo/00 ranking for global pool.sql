...
update v7 set v7.new_allocated_target = vf.final_target  from PROP_RANK_Sector_240_v20190212 v7
left join target_final vf on v7.prop_id = vf.prop_id where v7.new_allocated_target >0
...
update pr2
set pr2.global_rank=A.rnk from PROP_RANK_Sector_240_v20190212 pr2 inner join (
select  prop_id, dense_rank() over(partition by proposed_district  ORDER BY TotalScore desc,category ) rnk 
from PROP_RANK_Sector_240_v20190212 pr2
where  New_Allocated_Target is  null and pr2.rnk is not null 
...
--select * 
from PROP_RANK_Sector_240_v20190212 p inner join 
...
select isnull((SELECT  IS_NSDCPartner  FROM   [Pia_NSDCAssociation] PN
		...
		,Aspirational
		,PMKVY
		,PMKK from PROP_RANK_Sector_240_v20190212 pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id and pr.New_Allocated_Target is null
...
where p.global_Priority is null and New_Allocated_Target is null and p.rnk is not null 
