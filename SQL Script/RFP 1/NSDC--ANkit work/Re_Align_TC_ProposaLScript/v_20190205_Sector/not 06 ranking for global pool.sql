...
update pr2
set pr2.global_rank=A.rnk from PROP_RANK_Sector_240 pr2 inner join (
select  prop_id, dense_rank() over(partition by proposed_district  ORDER BY TotalScore desc,category ) rnk 
from PROP_RANK_Sector_240 pr2
where  isnull(New_Allocated_Target,0)=0 and pr2.rnk is not null 
)A on A.prop_id =pr2.prop_id --(3962 row(s) affected)
...
--select * 
from PROP_RANK_Sector_240 p inner join 
...
select isnull((SELECT  IS_NSDCPartner  FROM   [Pia_NSDCAssociation] PN
		...
		,Aspirational
		,PMKVY
		,PMKK from PROP_RANK_Sector_240 pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id and  isnull(pr.New_Allocated_Target,0)=0
...
where p.global_Priority is null and  isnull(New_Allocated_Target,0)=0 and p.rnk is not null 
