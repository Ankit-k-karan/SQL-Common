SELECT State_Name                                      AS [State],
       ...
       CategoryAlias_Name                              AS [Category],
		...
		,ISCompleteDD
		  ...
		  ,Replace((case  When (ISCompleteDD = 1 and Isnull(Prop_DD.L2Status,'') = '') then 'Save As Draft'
		   else Isnull(Prop_DD.L2Status,'')end),'Document Re-uploaded','Information Awaited')  [DDStatus on (2019-01-10)] 
		...
		, case When isnull(New_Allocated_Target,0) >0 then isnull(New_Allocated_Target,0)
		  else   isnull(pr.allocated_target,0)	end as [New Allocated target]
...
Left Join Prop_InitScreening Prop_DD ON v.proposalid = Prop_DD.Proposal_Id
