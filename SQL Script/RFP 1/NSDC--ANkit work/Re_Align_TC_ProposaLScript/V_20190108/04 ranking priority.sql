...
--select pm.id from prop_mainmaster pm  inner join PROP_RANK_v240 pr on pr.prop_id = pm.id where pm.proposal_code in ('NSDC_TP_1440720180901011700'
...
update v
set suspended_TP=1 from PROP_RANK_v240 v inner join prop_mainmaster p on v.prop_id = p.id
...
set p.Proposal_Priority =b.prir
 from PROP_RANK_v240 p inner join 
...
from PROP_RANK_v240 pr
		...
		update p
		set Valid_TC =0 from cte inner join PROP_RANK_v240 p on cte.prop_id = p.prop_id  where rrr>1 and Valid_TC =1