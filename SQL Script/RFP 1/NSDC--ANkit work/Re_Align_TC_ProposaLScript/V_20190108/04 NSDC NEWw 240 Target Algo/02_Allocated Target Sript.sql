   ...
   BEGIN
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			...
			set @MaxTarget = case when @District_Category in (1,2) then 2000  else 1200 end; --CHANGE 27.09.2018 --when @District_Category  =8 THEN 900 
			   set  @TargetAllocated = isnull((select sum(new_allocated_target) from PROP_RANK_v240 where proposed_district= @proposed_district  ),0);
