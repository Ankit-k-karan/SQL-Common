--select * into PROP_RANK_v240_final_20190212 from PROP_RANK_v5 v2 where new_allocated_target>0 and  new_allocated_target != allocated_target

select sum(allocated_target)allocated_target,sum(new_allocated_target)new_allocated_target,sum(old_allocated_target)old_allocated_target  from PROP_RANK_v240_final_20190212
update  PROP_RANK_v240_final_20190212 set  Global_rank = null ,  Global_priority = null

--Alter table PROP_RANK_v240_final_20190212 
...
--update v2
--set v2.allocated_target =v5.allocated_target , v2.iscompleteDD =v5.iscompleteDD
-- from PROP_RANK_v240_final_20190212 v2 inner join  PROP_RANK_v5 v5 on v2.prop_id = v5.prop_id

 select * from PROP_RANK_v240_final_20190212 Where iscompleteDD = 1 And  ISnull(New_Allocated_Target,0) = 0

 --select 902 -1116
 go
UPDATE PROP_RANK_v240_final_20190212
SET    New_Allocated_Target  =Null
where iscompleteDD is null

go

--select * from  PROP_RANK_v240_final_20190212_20190107 Where  Prop_ID In ( select Prop_ID from PROP_RANK_v240_final_20190212 Where iscompleteDD = 1 And  ISnull(New_Allocated_Target,0) = 0)
...
-- from PROP_RANK_v240_final_20190212 v240
--    inner join  PROP_RANK_v240_final_20190212 v240_20190107 on v240.Prop_ID =  v240_20190107.Prop_ID
...
-- select v2.* 
from PROP_RANK_v240_final_20190212 v2 inner join [vw_NSDC_SubmitProposalList] v on v2.prop_id = v.proposalid
...
set v2.New_Allocated_Target =v2.allocated_target 
 --select * 
 from PROP_RANK_v240_final_20190212 v2 where  v2.iscompleteDD is null and v2.category = 'Ct1'


PRINT '-------- Allocate target to Proposal --------';  
declare Category_1 CURSOR FOR
select distinct pr.category		
	 from PROP_RANK_v240_final_20190212 pr 
	 ...
	 from PROP_RANK_v240_final_20190212 pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id and pr.rnk is not null  --and  pr.proposed_district=301
		 inner join  Pia pia  on pia.Id = pm.Applicant_Pia_Id --and pr.prop_id =41151
		          and not exists (select 1 from PROP_RANK_v240_final_20190212 pr3
    ...
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			...
			@TCAllocated bigint =0;
			   ...
			   set  @TargetAllocated = isnull((select sum(allocated_target) from PROP_RANK_v240_final_20190212 where proposed_district= @proposed_district  ),0);
			   set  @TCAllocated = isnull((select count(1) from PROP_RANK_v240_final_20190212 pr
