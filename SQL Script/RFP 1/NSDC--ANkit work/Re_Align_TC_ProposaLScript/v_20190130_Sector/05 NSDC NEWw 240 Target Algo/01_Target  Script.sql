select  count(1) from PROP_RANK_Sector_240 v2 where new_allocated_target>0 or allocated_target >0  and  new_allocated_target != allocated_target
update  PROP_RANK_Sector_240 set Allocated_Target = 0 ,new_allocated_target =0 ,  Global_rank = null ,  Global_priority = null

--Alter table PROP_RANK_Sector_240 
...
from PROP_RANK_Sector_240 v2 inner join
...
set v2.New_Allocated_Target =v2.allocated_target 
 --select * 
 from PROP_RANK_Sector_240 v2 where isnull( v2.New_Allocated_Target,0) =0 and v2.allocated_target >0

 -- select * from PROP_RANK_Sector_240 Where iscompleteDD = 1 And  ISnull(New_Allocated_Target,0) = 0
...
--select * from  PROP_RANK_Sector_240_20190107 Where  Prop_ID In ( select Prop_ID from PROP_RANK_Sector_240 Where iscompleteDD = 1 And  ISnull(New_Allocated_Target,0) = 0)
...
-- from PROP_RANK_Sector_240 v240
...
---- select v2.* 
--from PROP_RANK_Sector_240 v2 inner join [vw_NSDC_SubmitProposalList] v on v2.prop_id = v.proposalid
--where  ISCompleteDD = 1 and allocated_target between  1 and  239 and v.proposed_target>=240

--update v2
--set v2.New_Allocated_Target =v2.allocated_target 
-- --select * 
-- from PROP_RANK_Sector_240 v2 where  v2.New_Allocated_Target =0 and v2.category = 'Ct1'
...
--		          and not exists (select 1 from PROP_RANK_Sector_240 pr3
...
--			   set  @TargetAllocated = isnull((select sum(New_Allocated_Target) from PROP_RANK_Sector_240 where proposed_district= @proposed_district  ),0);
--			   set  @TCAllocated = isnull((select count(1) from PROP_RANK_Sector_240 pr
