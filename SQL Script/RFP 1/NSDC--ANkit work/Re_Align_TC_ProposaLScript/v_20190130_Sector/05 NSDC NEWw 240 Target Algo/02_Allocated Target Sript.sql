...
select distinct pr.proposed_district	,District_priority	
	 from PROP_RANK_Sector_240 pr 
		inner join md_district_category dc on dc.district_id =pr.proposed_district -- and  pr.proposed_district=301
where Valid_TC =1  --and pr.category ='Ct1' 
and isnull(new_allocated_target,0)=0 and pr.rnk is not null
...
and Valid_TC = 1
and isnull(suspended_TP,0)=0 and  isnull(new_allocated_target,0)=0 --new_allocated_target is null
   ...
   BEGIN
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			...
			set @MaxTarget = case when @District_Category in (1,2) then 20000  else 12000 end; --CHANGE 27.09.2018 --when @District_Category  =8 THEN 900 
