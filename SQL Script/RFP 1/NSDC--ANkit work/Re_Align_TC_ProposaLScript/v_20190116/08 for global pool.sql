...
select distinct pr.proposed_district	,District_priority	
	 from Prop_Rank_v20190116 pr 
		inner join md_district_category dc on dc.district_id =pr.proposed_district
where Valid_TC =1  --and pr.category ='Ct1' 
and New_Allocated_Target is null and pr.Global_rank is not  null
--and  exists (select 1  from  Prop_Rank_v20190116 v
--					 where v.proposed_district = pr.proposed_district
--					 group by v.proposed_district 
--					 having case when dc.District_Category in (1,2) then 1000 else 600 end < sum( ( CASE
...
select (SELECT Count(DISTINCT pt.Trade_Id)
	 ...
	 from Prop_Rank_v20190116 pr
		inner join Prop_mainmaster pm on pm.id = pr.prop_id and pr.rnk is not null
		 ...
		 and not exists (select 1 from Prop_Rank_v20190116 pr3
...
and Valid_TC = 1
and isnull(suspended_TP,0)=0 and New_Allocated_Target is null
...
and (select tt_all from (select sum ( CASE
             WHEN Isnull(New_Allocated_Target, 0) > 0 THEN Isnull(New_Allocated_Target, 0)
             ELSE Isnull(v.allocated_target, 0)
           END ) tt_all from  Prop_Rank_v20190116 v
				 where v.proposed_district =   @proposed_district )a )< case when dc.District_Category in (1,2) then 1000 else 600 end 
and (select tt_all from (select count (New_Allocated_Target) tt_all from  Prop_Rank_v20190116 v
				 where v.proposed_district =   @proposed_district and  v.global_rank is not null )a ) < case when dc.District_Category in (1,2) then 1000 
																											when dc.District_Category in (3,4,5,6,7) then 2 else 1 end
    ...
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			...
			set @MaxTarget = case when @District_Category in (1,2) then 2000  when @District_Category  =8 THEN 900 else 1200 end; --CHANGE 27.09.2018
			   set  @TargetAllocated = isnull((select sum(New_Allocated_Target) from Prop_Rank_v20190116 where proposed_district= @proposed_district  ),0);
			   set  @TCAllocated = isnull((select count(1) from Prop_Rank_v20190116 pr
			   ...
			   update Prop_Rank_v20190116
			   set New_Allocated_Target = case when isnull(@TargetAllocated,0) + isnull(@Target,0) > @MaxTarget then 0 --- target can't exceed threshold limit 
