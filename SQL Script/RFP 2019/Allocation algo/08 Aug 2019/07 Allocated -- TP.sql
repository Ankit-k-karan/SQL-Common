...
--select * from  PROP_RANK where Allocated_Target is not null and category ='TP'

--update PROP_RANK set Allocated_Target=Null where Allocated_Target is not null and category ='TP' 
--select * from VW_NSDC_DISTRICT_PRIORITY
...
select  (SELECT Count(DISTINCT pt.Trade_Id)
 ...
 from Prop_MainMaster pm inner join PROP_RANK r on r.prop_id = pm.id and  pm.proposed_district =  @proposed_district and isnull(allocated_target,0)=0
...
order by  rnk,proposal_priority  ,org_type ,pm.Proposed_Target desc
   ...
   BEGIN
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @TargetperJob bigint=240,			
			@TargetAllocated bigint =0,
			@Target bigint =0,
			@TCAllocated bigint =0
			,@EligibleTarget bigint = 0;

			   set  @TargetAllocated = isnull((select sum(allocated_target) from PROP_RANK pr
			   ...
			   set  @TCAllocated = isnull((select count(1) from PROP_RANK pr
				...
				set  @EligibleTarget = case when (@TargetperJob * (case when @NoProposedTrade>=3 then 2 else @NoProposedTrade end)) >= @Proposed_Target then @Proposed_Target
											else  (@TargetperJob * (case when @NoProposedTrade>=3 then 2 else @NoProposedTrade end)) end
