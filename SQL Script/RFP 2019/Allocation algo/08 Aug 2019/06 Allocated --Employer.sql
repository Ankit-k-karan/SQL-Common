...
isnull((select Ans from  Pia_Questioner Where Is_Active = 1 And  Question = 'Q47' And Org_Id = pm.org_id ),0) employer_strength
 from Prop_MainMaster pm inner join PROP_RANK r on r.prop_id = pm.id-- and r.proposed_district = 445
...
--select * from  PROP_RANK where Allocated_Target is not null and category ='EMP'

--update PROP_RANK set Allocated_Target=Null where Allocated_Target is not null and category ='EMP' 
