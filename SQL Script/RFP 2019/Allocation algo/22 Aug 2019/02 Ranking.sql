...
select *  from PROP_RANK_0822 r
select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_0822 r
group by category,Org_Type,proposed_district,rnk
having count(1)>1
order by 1
--drop table PROP_RANK_0822

 if not exists (select * from sys.TAbles Where NAme = 'PROP_RANK_0822')
 begin
 create table PROP_RANK_0822
 ...
 -- truncate table PROP_RANK_0822

 ---############### Insert 14990 proposals with initial ranking
  insert into PROP_RANK_0822 (prop_id,category,[Org_Type],proposed_district,TotalScore,rnk)  
...
update t set t.Valid_TC= tc.maxTC ,rnk =0 from PROP_RANK_0822 t inner join Prop_MainMaster pm on pm.id = t.prop_id 
 inner join pia ap on ap.id =pm.Applicant_Pia_Id  and  CASE  WHEN AP.EmployerOrganizationType = 'EMPLOYER_CONSORTIUM' OR AP.ApplicantType = 'EMPLOYER' THEN ( CASE
         ...
         ELSE ''
       END =''
 inner join NSDC_Allowed_TC_0822 tc on tc.Applicant_Pia_Id = pm.applicant_pia_id 
...
update r set rnk = a.RNKS from (	select rank() over (partition by Org_Type,  Proposed_District		 
		 	 order by Isnull(TotalScore,0) desc)  RNKS, prop_id from  PROP_RANK_0822  r where   category ='TP' and Valid_TC >0 AND RNK=0
	)A inner join   PROP_RANK_0822  r on a.prop_id = r.prop_id and   category ='TP' and Valid_TC >0 AND RNK=0	  ---(12085 row(s) affected)
...
update r set Proposal_Priority = RNK_priorty from ( select  rank() over (partition by t.Proposed_District,rnk		 
		 	 order by prop_id)  RNK_priorty, prop_id  from PROP_RANK_0822 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category='EMp'
 where exists (select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_0822 r
 where t.proposed_district = r.proposed_district and t.rnk = r.rnk and t.category = r.category
group by category,Org_Type,proposed_district,rnk
having count(1)>1
) )a inner join   PROP_RANK_0822  r on a.prop_id = r.prop_id and   category ='EMp' and Proposal_Priority is null
...
update r set Proposal_Priority = pr_rnk
 --select * 
  from ( select  rank() over (partition by t.Proposed_District, org_type	,rnk	 
			 ...
			 from PROP_RANK_0822 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category!='EMp' and rnk >0 
...
--select * from PROP_RANK_0822  where proposed_district =430 and   rnk >0 and category! ='EMp' and Org_Type='NEW'

--select category,Org_Type,proposed_district,rnk ,Proposal_Priority,count(1) cnt from PROP_RANK_0822 r
