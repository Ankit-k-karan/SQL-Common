...
isnull((select Ans from  Pia_Questioner Where Is_Active = 1 And  Question = 'Q47' And Org_Id = pm.org_id ),0) employer_strength
 from Prop_MainMaster pm inner join PROP_RANK_0904 r on r.prop_id = pm.id-- and r.proposed_district = 445
...
--select * from  PROP_RANK_0904 where Allocated_Target is not null and category ='EMP'

--update PROP_RANK_0904 set Allocated_Target=Null where Allocated_Target is not null and category ='EMP' 
...
select  (SELECT Count(DISTINCT pt.Trade_Id)
 ...
 from Prop_MainMaster pm inner join PROP_RANK_0904 r on r.prop_id = pm.id and  pm.proposed_district =  @proposed_district and isnull(allocated_target,0)=0
inner join pia p on Applicant_Pia_Id= p.id 
inner join md_districts d on d.id = pm.proposed_district where r.category ='EMP'
    ...
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			...
			,@EligibleTarget bigint = 0;
			   ...
			   set  @TargetAllocated = isnull((select sum(allocated_target) from PROP_RANK_0904 pr
			   ...
			   set  @TCAllocated = isnull((select count(1) from PROP_RANK_0904 pr
											inner join Prop_mainmaster pm on pm.id = pr.prop_id and allocated_target>0
													 where pm.applicant_pia_id  in  (select applicant_pia_id from  Prop_mainmaster where id =@prop_id)  ),0);
