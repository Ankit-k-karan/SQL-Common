...
isnull((select Ans from  Pia_Questioner Where Is_Active = 1 And  Question = 'Q47' And Org_Id = pm.org_id ),0) employer_strength
 from Prop_MainMaster pm inner join PROP_RANK_0912_1000 r on r.prop_id = pm.id-- and r.proposed_district = 445
inner join pia p on Applicant_Pia_Id= p.id 
inner join VW_NSDC_DISTRICT_PRIORITY_v2 dc on dc.DistrictName =r.proposed_district
inner join md_districts d on d.id = pm.proposed_district where r.category ='EMP'
order by 1,dc.SeqNo,dc.id 


--select * from  PROP_RANK_0912_1000 where Allocated_Target is not null and category ='EMP'

--update PROP_RANK_0912_1000 set Allocated_Target=Null where Allocated_Target is not null and category ='EMP' 
--select * from VW_NSDC_DISTRICT_PRIORITY_v2

PRINT '-------- Allocate target to Employer--------';  
declare CUR_EMP CURSOR FOR
select distinct pr.proposed_district,dc.SeqNo,dc.DistrictCategory	
	 from PROP_RANK_0912_1000 pr 
		inner join VW_NSDC_DISTRICT_PRIORITY_v2 dc on dc.DistrictName =pr.proposed_district
where pr.category ='EMP'
and pr.rnk is not null and  isnull(allocated_target,0)=0 --and proposed_district = 445
order by dc.DistrictCategory,dc.SeqNo
...
select  (SELECT Count(DISTINCT pt.Trade_Id)
 ...
 from Prop_MainMaster pm inner join PROP_RANK_0912_1000 r on r.prop_id = pm.id and  pm.proposed_district =  @proposed_district and isnull(allocated_target,0)=0
inner join pia p on Applicant_Pia_Id= p.id 
inner join md_districts d on d.id = pm.proposed_district where r.category ='EMP'
   ...
   BEGIN
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @MaxTarget bigint =0,
			@TargetperTC bigint=1000,
			@MaxTC bigint= 500,
			@TargetAllocated bigint =0,
			@Target bigint =0,
			@TCAllocated bigint =0
			,@EligibleTarget bigint = 0;
				set @MaxTarget =  1000000;
			   set  @TargetAllocated = isnull((select sum(allocated_target) from PROP_RANK_0912_1000 pr
			   ...
			   set  @TCAllocated = isnull((select count(1) from PROP_RANK_0912_1000 pr
											inner join Prop_mainmaster pm on pm.id = pr.prop_id and allocated_target>0
													 where pm.applicant_pia_id  in  (select applicant_pia_id from  Prop_mainmaster where id =@prop_id)  ),0);
