...
select *  from PROP_RANK_0819 r
select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_0819 r
group by category,Org_Type,proposed_district,rnk
having count(1)>1
order by 1
--drop table PROP_RANK_0819

 if not exists (select * from sys.TAbles Where NAme = 'PROP_RANK_0819')
 begin
 create table PROP_RANK_0819
 ...
 -- truncate table PROP_RANK_0819

 ---############### Insert 14990 proposals with initial ranking
  insert into PROP_RANK_0819 (prop_id,category,[Org_Type],proposed_district,TotalScore,rnk)  
...
update t set t.Valid_TC= tc.maxTC ,rnk =0 from PROP_RANK_0819 t inner join Prop_MainMaster pm on pm.id = t.prop_id 
 inner join pia ap on ap.id =pm.Applicant_Pia_Id  and  CASE  WHEN AP.EmployerOrganizationType = 'EMPLOYER_CONSORTIUM' OR AP.ApplicantType = 'EMPLOYER' THEN ( CASE
         ...
         ELSE ''
       END =''
 inner join NSDC_Allowed_TC_0819 tc on tc.Applicant_Pia_Id = pm.applicant_pia_id 
 ...
 --- ##### realloction of ranks to TP type proposals.
update r set rnk = a.RNKS from (	select rank() over (partition by Org_Type,  Proposed_District		 
		 	 order by Isnull(TotalScore,0) desc)  RNKS, prop_id from  PROP_RANK_0819  r where   category ='TP' and Valid_TC >0 AND RNK=0
	)A inner join   PROP_RANK_0819  r on a.prop_id = r.prop_id and   category ='TP' and Valid_TC >0 AND RNK=0	  ---(13112 row(s) affected)
...
update r set Proposal_Priority = RNK_priorty from ( select  rank() over (partition by t.Proposed_District,rnk		 
		 	 order by prop_id)  RNK_priorty, prop_id  from PROP_RANK_0819 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category='EMp'
 where exists (select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_0819 r
 where t.proposed_district = r.proposed_district and t.rnk = r.rnk and t.category = r.category
group by category,Org_Type,proposed_district,rnk
having count(1)>1
) )a inner join   PROP_RANK_0819  r on a.prop_id = r.prop_id and   category ='EMp' and Proposal_Priority is null

select * from NSDC_PriorityFactor where  category!='EMp' 
  --- allocating proposal priority as need to have some sequence for TP --5359
update r set Proposal_Priority = pr_rnk
 --select * 
  from ( select  rank() over (partition by t.Proposed_District, org_type	,rnk	 
			 ...
			 from PROP_RANK_0819 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category!='EMp' and rnk >0 
...
update  PROP_RANK_0819
set Proposal_Priority =1  where  Proposal_Priority is null and rnk >0

--select * from PROP_RANK_0819  where proposed_district =430 and   rnk >0 and category! ='EMp' and Org_Type='NEW'

--select category,Org_Type,proposed_district,rnk ,Proposal_Priority,count(1) cnt from PROP_RANK_0819 r
...
group by category,Org_Type,proposed_district,rnk
having count(1)>1
order by  category, proposed_district, rnk