...
--select *  from PROP_RANK_1109 r
-- where  category ='P5'

 --update PROP_RANK_1109
 --set allocated_target =Null
--drop table PROP_RANK_1109

 if not exists (select * from sys.TAbles Where NAme = 'PROP_RANK_1109')
 begin
 create table PROP_RANK_1109
 ...
 -- truncate table PROP_RANK_1109

 ---############### Insert (8253 row(s) affected)
  insert into PROP_RANK_1109 (prop_id,category,proposed_district,rnk)  
...
WHERE  PropMain.Status = 1
	   ...
	   and Proposal_Code  not in (select Proposal_Code from Prop_MainMaster_20200922)
and 	not exists (select 1 from PROP_RANK_1109 r where r.prop_id =PropMain.Id) ;



 ---cancel TC --(291 row(s) affected)
-- update PROP_RANK_1109 set CancelTC =Null
update r set canceltc =1
 from  pia p 
  inner join  Prop_MainMaster pm WITH(nolock) on p.id = pm.applicant_pia_id and p.isexistingpmkvytp=1
   inner join PROP_RANK_1109 r on r.prop_id = pm.id and r.category in ('P1','P2','P5')
...
update r set Placed_per= case when No_of_candidates_certified=0 then 0 else  round((no_of_candidates_placed *100 /No_of_candidates_certified),2) end , valid_tc=0
 ...
 inner join PROP_RANK_1109 r on r.prop_id = pm.id and r.category in ('P1','P3','P6');
...
update r set Placed_per= case when No_of_candidates_certified=0 then 0 else  round((no_of_candidates_placed *100 /No_of_candidates_certified),2) end ,valid_tc =1
 ...
 from  pia p 
 inner join  Prop_MainMaster pm WITH(nolock) on p.id = pm.applicant_pia_id and p.isexistingpmkvytp=1
 inner join PROP_RANK_1109 r on r.prop_id = pm.id and r.category not in ('P1','P3','P6') 
...
from Prop_Placement_Details pd inner join PROP_RANK_1109 pr on pr.prop_id = pd.prop_id where is_active=1 and rnk =0-- and isnull(Placed_per,0) =0 
 and pr.category='P6'

 select category,count(1) from PROP_RANK_1109 group by category
...
from (	select rank() over (partition by   Proposed_District,category		 
		 	 order by isnull(cancelTC,0) desc, Isnull(totalscore,0) desc)  RNKS, prop_id from  PROP_RANK_1109  r where placed_per is not null --and category not  in ('P1', 'P3' ,'P6' )
			  AND RNK=0
	)A inner join   PROP_RANK_1109  r on a.prop_id = r.prop_id and placed_per is not null --and category  in ('P1', 'P3' ,'P6' ) 
	AND Isnull(RNK,0)=0 ---(688 row(s) affected) and (7565 row(s) affected)
...
update r set Proposal_Priority = RNK_priorty from ( select  rank() over (partition by t.Proposed_District,category,rnk		 
			  ...
			  from PROP_RANK_1109 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and  rnk>0
			-- inner join NSDC_SDMS_TC_DETAIL  d  on (d.tc_smart_id = pm.SDMSTCCode2 or d.tc_smart_id = pm.SDMSTCCode)
 where exists (select category,proposed_district,rnk ,count(1) cnt from PROP_RANK_1109 r where rnk>0
...
having count(1)>1
) )a inner join   PROP_RANK_1109  r on a.prop_id = r.prop_id and   Isnull(r.rnk,0)>0 and Proposal_Priority is null --and r.valid_tc=0
...
--			  from PROP_RANK_1109 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and  rnk>0 and valid_tc=1
...
--group by category,proposed_district,rnk
--having count(1)>1
--) )a inner join   PROP_RANK_1109  r on a.prop_id = r.prop_id and   r.rnk>0 and r.Proposal_Priority is null and r.valid_tc=1

--- update capacity as per sheet shared. 
--(230 row(s) affected)


 --  TCRoleCapcity Max Capacity not given
--update pr set maxcapacityTC = a.cap
----select * 
--from PROP_RANK_1109 pr inner join(
--select t.prop_id, sum(trc.capacity)cap  from PROP_RANK_1109 t
...
--select * 
from PROP_RANK_1109 pr inner join(
select t.prop_id, pm.proposed_target  from PROP_RANK_1109 t
  inner join Prop_MainMaster pm on pm.id = t.prop_id  and  rnk>0
...
--			 from PROP_RANK_1109 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category!='EMp' and rnk >0 
--			inner join NSDC_PriorityFactor pf on pf.prop_id =pm.id 				
-- where exists (select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_1109 r
...
--having count(1)>1
--) )a inner join   PROP_RANK_1109  r on a.prop_id = r.prop_id and   category! ='EMp' and Proposal_Priority is null
----order by proposed_district

--update  PROP_RANK_1109
--set Proposal_Priority =1  where  Proposal_Priority is null and rnk >0 --7646

--select * from PROP_RANK_1109  where proposed_district =430 and   rnk >0 and category! ='EMp' and Org_Type='NEW'

--select category,Org_Type,proposed_district,rnk ,Proposal_Priority,count(1) cnt from PROP_RANK_1109 r
-- where    rnk >0 and category! ='EMp'
--group by category,Org_Type,proposed_district,rnk,Proposal_Priority
--having count(1)>1

select category,proposed_district,rnk ,Proposal_Priority,count(1) cnt from PROP_RANK_1109 r
 where rnk>0
