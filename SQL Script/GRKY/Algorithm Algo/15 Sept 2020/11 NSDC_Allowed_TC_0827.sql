--(1454 row(s) affected)
--select * from NSDC_Allowed_TC_0827
select Applicant_Pia_Id
		...
		, case when TPOrganizationType = 'NEW' and profit_tc>0 then profit_tc
				...
				else per_tc end 	maxTC ---into  NSDC_Allowed_TC_0827 
				from (
select Applicant_Pia_Id
		,Turnove
		,TPOrganizationType
	 ...
	 AP.TPOrganizationType,
       Isnull((SELECT CASE Count(finyear_turnover)
               ...
               WHERE  pia_fin.Id IN (SELECT TOP 3 pf.Id
                                     ...
                                     ORDER  BY pf.finyear_start_date DESC)), 0) AS Turnove, --select * from NSDC_SDMS_TP_DETAIL
