...
select *  from PROP_RANK_0827 r
select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_0827 r
group by category,Org_Type,proposed_district,rnk
having count(1)>1
order by 1
--drop table PROP_RANK_0827

 if not exists (select * from sys.TAbles Where NAme = 'PROP_RANK_0827')
 begin
 create table PROP_RANK_0827
 ...
 -- truncate table PROP_RANK_0827

 ---############### Insert 14990 proposals with initial ranking
  insert into PROP_RANK_0827 (prop_id,category,[Org_Type],proposed_district,TotalScore,rnk)  
...
update t set t.Valid_TC= tc.maxTC ,rnk =0 from PROP_RANK_0827 t inner join Prop_MainMaster pm on pm.id = t.prop_id 
...
update r set rnk = a.RNKS from (	select rank() over (partition by Org_Type,  Proposed_District		 
		 	 order by Isnull(TotalScore,0) desc)  RNKS, prop_id from  PROP_RANK_0827  r where   category ='TP' and Valid_TC >0 AND RNK=0
	)A inner join   PROP_RANK_0827  r on a.prop_id = r.prop_id and   category ='TP' and Valid_TC >0 AND RNK=0	  ---(12085 row(s) affected)
...
update r set Proposal_Priority = RNK_priorty from ( select  rank() over (partition by t.Proposed_District,rnk		 
		 	 order by prop_id)  RNK_priorty, prop_id  from PROP_RANK_0827 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category='EMp'
 where exists (select category,Org_Type,proposed_district,rnk ,count(1) cnt from PROP_RANK_0827 r
 where t.proposed_district = r.proposed_district and t.rnk = r.rnk and t.category = r.category
group by category,Org_Type,proposed_district,rnk
having count(1)>1
) )a inner join   PROP_RANK_0827  r on a.prop_id = r.prop_id and   category ='EMp' and Proposal_Priority is null
...
update r set Proposal_Priority = pr_rnk
 --select * 
  from ( select  rank() over (partition by t.Proposed_District, org_type	,rnk	 
			 ...
			 from PROP_RANK_0827 t inner join Prop_MainMaster pm on pm.id = t.prop_id  and category!='EMp' and rnk >0 
...
update  PROP_RANK_0827
set Proposal_Priority =1  where  Proposal_Priority is null and rnk >0 --7646

--select * from PROP_RANK_0827  where proposed_district =430 and   rnk >0 and category! ='EMp' and Org_Type='NEW'

--select category,Org_Type,proposed_district,rnk ,Proposal_Priority,count(1) cnt from PROP_RANK_0827 r
