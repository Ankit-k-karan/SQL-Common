...
--select * from  PROP_RANK_0917 where rnk>0   and  isnull(allocated_target,0)>0  and maxcapacityTc is not null 

PRINT '-------- Allocate target to TP--------';  
declare CUR_EMP CURSOR FOR
select distinct pr.proposed_district		
	 from PROP_RANK_0917 pr 
...
select  (SELECT Count(DISTINCT pt.Trade_Id)
		...
		,r.MaxCapacityTC 
		,dt.districttarget
 from Prop_MainMaster pm inner join PROP_RANK_0917 r on r.prop_id = pm.id and  pm.proposed_district =  @proposed_district and isnull(allocated_target,0)=0
    ...
    --  Print 'Proposal Id - ' @prop_id 
	  ...
	  declare @TargetperJob bigint=30,			
			...
			,@EligibleTarget bigint = 0;
			   ...
			   set  @TargetAllocated = isnull((select sum(allocated_target) from PROP_RANK_0917 pr
											inner join Prop_mainmaster pm on pm.id = pr.prop_id and allocated_target>0 
													 where pm.proposed_district  = @proposed_district  ),0);
			   set  @TCAllocated = isnull((select count(1) from PROP_RANK_0917 pr
											inner join Prop_mainmaster pm on pm.id = pr.prop_id and allocated_target>0 
