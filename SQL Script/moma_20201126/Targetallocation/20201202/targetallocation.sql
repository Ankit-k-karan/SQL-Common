...
update prop_target_allocation_02122020 set allocatedtarget=0,remarks=null
...
declare @tpallocatedtarget bigint=0,@isused bit,@FinalTarget bigint=0,@Target bigint,@JobroleallocatedTarget bigint,@districtallocatedtarget bigint,@remarks nvarchar(max)
	...
	select @tpallocatedtarget=sum(isnull(allocatedtarget,0)) from prop_target_allocation_02122020 a 
	                         --inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode
                               --   left  join tp_ranking_startrating b on b.prn=a.tp_code 
							   where a.tpid=@tpid 
							   --and c.isactive=1
   ...
   set @JobroleallocatedTarget=(select sum(isnull(allocatedtarget,0)) from prop_target_allocation_02122020 a-- inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode
                                  --left  join tp_ranking_startrating b on b.prn=a.tp_code 
								  where a.tradeid=@tradeid )
								  --and c.isactive=1 )

	set @districtallocatedtarget=(select sum(isnull(allocatedtarget,0)) from prop_target_allocation_02122020 a 
	--inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode
                                  --left  join tp_ranking_startrating b on b.prn=a.tp_code 
...
update prop_target_allocation_02122020 set allocatedtarget=@Target,remarks =@remarks

  where proposalid=@proposalid and tradeid=@tradeid and districtid=@proposed_district
 
 --insert into prop_target_allocation_02122020(districtid,tpid,tpname,tp_code,proposalid,orgid,proposedtarget,jobcode,tradeid,totalscore,tprank,p1,allocatedTarget)
