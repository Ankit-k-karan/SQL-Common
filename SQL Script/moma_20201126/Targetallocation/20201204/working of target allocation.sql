...
select sum(allocatedtarget) from prop_target_allocation_02122020


select * from prop_target_allocation_02122020 where allocatedtarget>0 and districtid=186 order by totalscore desc
...
from prop_target_allocation_02122020 a 
inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode and c.isactive=1
...
select * from prop_target_allocation_02122020 where allocatedtarget>0 and tradeid=1371 order by totalscore desc

select * from temp2 where  propid =321tradeid=1371  order by districtid
--select * from prop_target_allocation_02122020 where allocatedtarget>0 and districtid=186 order by totalscore desc
select  tpid ,proposalid,proposedtarget,a.tradeid,a.districtid,a.tprank,isnull(b.tprank,99999)p1,totalscore,allocatedtarget from prop_target_allocation_02122020 a 
inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode and c.isactive=1
...
left join (
select districtid,sum(allocatedtarget)allo from prop_target_allocation_02122020
group by districtid
...
,sum(case when allocatedtarget=0 then 1 else 0 end)
  from prop_target_allocation_02122020 a 
inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode where a.tpid in(select a.tpid  from prop_target_allocation_02122020 a 
               inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode where a.districtid=184)
 ...
 select a.* from prop_target_allocation_02122020 a
 inner join prop_target_allocation_02122020 b on  b.tprank=a.tprank
 and a.districtid=b.districtid


 select districtid,tprank,count(*) from prop_target_allocation_02122020
 group by  districtid,tprank
 having count(*)>1

 --drop table temp_rank
 select distinct districtid,tpid,tprank,null p1,tp_code into temp_rank from 
 prop_target_allocation_02122020 --where districtid=183
 ...
 select a.*,b.tprank From prop_target_allocation_02122020 a
 left join temp_rank b on b.tp_code=a.tp_code and b.districtid=a.districtid
 inner join districtwise_jobrole_20201202  c on a.districtid=c.districtid and a.jobcode=c.tradecode and c.isactive=1
 ...
 where a.tpid=55
  ...
  select *  from districtwise_jobrole_20201203 where tradecode='SSC/Q2212'

  select distinct tradecode from districtwise_jobrole_20201202 where tradecode in (

  select distinct jobcode from prop_target_allocation_02122020 where remarks is null
  )
  and isactive=1
...
where a.jobcode='AMH/Q1947'
 ...
 select * From prop_target_allocation_02122020 where tp_code='NSDC2020MOMA00223'