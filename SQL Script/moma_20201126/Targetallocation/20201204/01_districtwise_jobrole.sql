...
--create table districtwise_jobrole_20201203
...
--truncate table districtwise_jobrole_20201203

-------------632
insert into districtwise_jobrole_20201203(districtid,districtname,tradeid,tradecode,totalhours)


select md.id,md.district_name,mt.id,mt.trade_code, totalhours
from (
...
union all SELECT 'Jammu' District, 'ASC/Q1103' QP_Code, 'Automotive' Sector, 'Automotive Showroom Hostess/Host' JobName, '200' totalHours   
...
union all SELECT 'Shopian' District, 'ASC/Q1103' QP_Code, 'Automotive' Sector, 'Automotive Showroom Hostess/Host' JobName, '200' totalHours   
...
inner join md_trades mt on mt.trade_code=t.qp_code and mt.is_active=1 and ltrim(rtrim(mt.Trade_Name))=ltrim(rtrim(t.JobName))
--where mt.Trade_Code in ('ASC/Q1401')
where not exists(Select 1 from districtwise_jobrole_20201203 a where a.districtname=t.District and a.tradecode=t.QP_Code)
